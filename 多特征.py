...
def myRNN (train_X, train_y,est_tX,test_y,timestep,feture):
    #单特征GRU，可循环
    model = Sequential()
    model.add(SimpleRNN(64,return_sequences=True,activation="relu",input_shape=(timestep,feture)))
...
for i in range(1,9):
    ...
    for j in range(1,9):
        timestep=j
        dirname=str(timestep)+"时间"+str(i)+"滚动"
        if timestep<rollstep:
            ...
            if  os.path.exists("picture/"+dirname):
                ...
                for art in data:
                    ...
                    model,history=myRNN(train_X, train_y,test_X,test_y,timestep,featurelie)
                    perdict=timeperdict(model,lasttrain,rollstep,timestep)
                    ...
                    font =FontProperties(fname='PingFang.ttc') #需要指定字体
...
for art in data:
    ...
    model,history=OFLSTM(train_X, train_y,test_X,test_y,timestep,featurelie)
    perdict= model.predict(lasttrain)
